...
namespace binapi
{
  namespace rest
  {
    ...
    struct api
    {
      ...
      api(
          boost::asio::io_context &ioctx, std::string host, std::string port, std::string pk, std::string sk, std::size_t timeout, std::string client_api_string = "binapi-0.0.1"); // constructor
      virtual ~api();
      ...
      result<orders_info_t>
      all_orders(
          const std::string &symbol, std::size_t orderid = 0, std::size_t start_time = 0, std::size_t end_time = 0, std::size_t limit = 0, all_orders_cb cb = {})
      {
        return all_orders(
            symbol.empty() ? nullptr : symbol.c_str(), orderid, start_time, end_time, limit, std::move(cb));
      }
      result<orders_info_t>
      all_orders(
          const char *symbol, std::size_t orderid = 0, std::size_t start_time = 0, std::size_t end_time = 0, std::size_t limit = 0, all_orders_cb cb = {});
      ...
      new_order(
          const std::string &symbol, const e_side side, const e_type type, const e_time time, const e_trade_resp_type resp, const std::string &amount, const std::string &price, const std::string &client_order_id, const std::string &stop_price, const std::string &iceberg_amount, new_order_cb cb = {})
      {
        return new_order(
            symbol.c_str(), side, type, time, resp, (amount.empty() ? nullptr : amount.c_str()), (price.empty() ? nullptr : price.c_str()), (client_order_id.empty() ? nullptr : client_order_id.c_str()), (stop_price.empty() ? nullptr : stop_price.c_str()), (iceberg_amount.empty() ? nullptr : iceberg_amount.c_str()), std::move(cb));
      }
      result<new_order_resp_type>
      new_order(
      ...
      cancel_order(
          const std::string &symbol, std::size_t order_id, const std::string &client_order_id, const std::string &new_client_order_id, cancel_order_cb cb = {})
      {
        return cancel_order(
            symbol.c_str(), order_id, (client_order_id.empty() ? nullptr : client_order_id.c_str()), (new_client_order_id.empty() ? nullptr : new_client_order_id.c_str()), std::move(cb));
      }
      result<cancel_order_info_t>
      ...
      my_trades(
          const std::string &symbol, std::size_t start_time, std::size_t end_time, std::size_t from_id, std::size_t limit, my_trades_cb cb = {})
      {
